...
| `tests/test_t2_real_email.py` | T2: multipart / Formspree-style / HTML-escaped JSON / 中文 |
...
|---|---|
| 公钥 PEM | contents of `keys/record_encrypt_public.pem` |
| 密钥 ID (kid) | e.g. `phone-form` — anything you chose in `new-kid` |
| 授权密钥 | the secret printed by `new-kid` (re-read it from `kid_secrets.json`) |
| 收件邮箱 | the same address as `imap.username` in `config.yaml` |

Also make sure the form's **主题前缀** equals `imap.subject_prefix` in
`config.yaml` (the frontend default `[OpenClaw Secure Record]` matches
the shipped example config). Send one test record, then:
...
* `crypto.private_key_path` — RSA-2048 PEM, generated by
  `python3 -c "from pathlib import Path; from src.sender import generate_keypair; generate_keypair(Path('keys'))"`.
